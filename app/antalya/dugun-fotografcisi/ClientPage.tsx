...
export function ClientPage() {
  ...
  // Structured data (kept here; could be moved to server for smaller bundle)
  ...
  return (
    <>
      ...
      <div className="pt-20">
        {/* Original page content moved from page.tsx */}
        {/* Hero Section */}
          ...
          <div className="container relative z-10">
            <div className="text-center max-w-4xl mx-auto">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-playfair font-bold mb-6">
                Antalya Düğün Fotoğrafçısı – Hikaye Odaklı Çekim
              </h1>
              ...
              {weddingLocations.map((location, index) => (
                <div key={index} className="bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 group">
                  <div className="relative h-64 overflow-hidden">
                    <Image 
                      src={location.image} 
                      alt={`${location.name} | Antalya düğün fotoğrafçısı, dış çekim, gelin damat, profesyonel düğün fotoğrafları`}
                      fill
        ...
        <section className="py-20 bg-gradient-to-b from-gray-50 to-white">
          <div className="container">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[
                { icon: Camera, title: 'Professional Ekipman', text: 'Canon 5D Mark IV, profesyonel objektifler, studio flashları ve drone ile her koşulda mükemmel kalite sunuyoruz.' },
                { icon: Heart, title: 'Hikaye Anlatımı', text: 'Sadece fotoğraf çekmiyoruz, aşk hikayenizi belgeleriz. Her kare duygusal bir anlam taşır ve yıllar sonra o anları yaşatır.' },
                { icon: MapPin, title: 'Lokasyon Uzmanlığı', text: 'Antalya\'nın her köşesini biliyoruz. Kaleiçi\'nin gizli köşelerinden Side\'nin antik güzelliğine kadar en iyi kareler için rehberlik ediyoruz.' },
                { icon: Clock, title: 'Hızlı Teslimat', text: 'Ön izleme fotoğraflarını 7 gün içinde, düzenlenmiş tüm kareler 4 hafta içinde online galerinizde.' },
                { icon: Users, title: 'Deneyimli Ekip', text: '10+ yıl tecrübe, 500+ mutlu çift. Premium paketlerde ikinci fotoğrafçı desteği ile hiçbir anı kaçırmıyoruz.' },
                { icon: Star, title: 'Kişisel Yaklaşım', text: 'Her çiftin hikayesi unique. Kişiliklerinize, tarzınıza uygun konseptler geliştiriyor, özel isteklerinizi dikkate alıyoruz.' }
              ].map((card, idx) => (
                <div key={idx} className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300">
                  <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-6">
                    <card.icon className="w-8 h-8 text-primary" />
                  </div>
                  <h3 className="text-xl font-semibold text-gray-900 text-center mb-4">{card.title}</h3>
          ...
          <div className="container">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div>
                ...
                <div className="space-y-6">
                  {[
                    { step: 1, title: 'Ön Görüşme & Planlama', text: 'Çiftlerimizle tanışıyor, hikayelerini dinliyor, beklentilerini öğreniyoruz. Düğün gününün programını detaylıca planlıyoruz.' },
                    { step: 2, title: 'Lokasyon Keşfi', text: 'Antalya\'da çekim yapılacak mekanları önceden inceliyoruz. En iyi ışık koşulları ve kompozisyon noktalarını belirliyoruz.' },
                    { step: 3, title: 'Çekim Günü', text: 'Hazırlıktan after party\'ye kadar düğün gününün her anında yanınızdayız. Doğal anları yakalar, grup fotoğraflarını organize ederiz.' },
                    { step: 4, title: 'Post-Production & Teslimat', text: 'Her fotoğrafı özenle seçip düzenliyoruz. Renk düzeltmesi, ton ayarları ve retüş çalışmalarıyla mükemmel sonuçlar elde ediyoruz.' }
                  ].map(item => (
          ...
          <div className="container">
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <div>
                ...
                <div className="space-y-4">
                  {[
                    { title: 'Golden Hour (Altın Saat)', text: 'Gün doğumundan 1 saat sonra ve gün batımından 1 saat önce. En romantik ışık koşulları.' },
                    { title: 'Mavi Saat (Blue Hour)', text: 'Gün batımından sonraki 30 dakika. Şehir ışıkları ve doğal ışığın mükemmel karışımı.' },
                    { title: 'İç Mekan Çekimleri', text: 'Öğle saatlerinde pencere ışığı kullanımı, gece çekimlerinde ambient ışık kombinasyonları.' }
                  ].map((tip, idx) => (
              ...
              <div>
                ...
                <div className="space-y-4">
                  {[
                    { title: 'Kaleiçi - Tarihi Doku', text: 'Osmanlı mimarisi, taş sokaklar, geleneksel evler. Klasik ve zarif kareler için ideal.' },
                    { title: 'Lara Beach - Sahil', text: 'Altın kumlar, masmavi deniz, gün batımı çekimleri. Romantik ve dramatik etkiler.' },
                    { title: 'Side - Antik', text: 'Roma tiyatrosu, antik sütunlar, tarihi kalıntılar. Unique ve görkemli kompozisyonlar.' }
                  ].map((loc, idx) => (
                    <div key={idx} className="bg-white rounded-xl p-6 shadow-md">
                      <h4 className="font-semibold text-gray-900 mb-2">{loc.title}</h4>
                      <p className="text-gray-600 text-sm">{loc.text}</p>
                    </div>
                  ))}
                </div>
              </div>
